...
const UnreadNotifications = ({ data }) => {
    ...
    const { setLoadPage,setShowDetailPostModal, setSelectedPostId, setErrorModal, setErrorMessage, setShowCommentModal, showCommentModal } = useNavigationUserProfileHooks();
                    ...
                    <>
                        {notification?.isRead === false ? (
                            <div className="p-1 border border-white/10 rounded-lg bg-card">
                                <div className="flex border border-white/10 rounded-lg bg-black/30 gap-2 md:gap-5 p-2 cursor-pointer" onClick={(e) => {handleReadNotification(notification._id, e), handleContentModal(notification)}}>
                                    {/* user profile */}
