...
const ReplycommentForm = ({ commentId }) => {
    ...
    const { handleUploadReplycomment, error, setError } = useReplyCommentServices()
    const { setShowReplycommentForm } = useNavigationUserProfileHooks()
    ...
    return (
        <>
            <div className="fixed top-0 left-0 right-0 bottom-0 flex justify-center items-center z-15 bg-transparent px-4 text-white" onClick={() => { handleDeleteInput(), setShowReplycommentForm(false)}}>
                <form className="p-3 flex flex-col rounded-lg w-full max-w-[500px] bg-black border border-white/50 self-end" onClick={(e) => e.stopPropagation()}>
                    <h1 className="self-center">Reply Comment</h1>
                    {/* Profile */}
                    <TextareaAutosize placeholder="Your Reply Comment" minRows={2} maxRows={25} className="border border-white/50 rounded-lg focus:ring-my-green focus:outline-none focus:ring-3 p-1 font-extralight" maxLength={characterLimit} onChange={(e) => setContent(e.target.value)} />
                    ...
                    <div className="flex justify-between">
                        ...
                        <div className="bg-my-green rounded-lg">
                            <button type="submit" className="px-4 py-2 text-lg font-bold cursor-pointer flex justify-center items-center" onClick={(e) => handleUploadReplycomment(content,selectedImage,commentId,e)}>
                                {/* <span>Send</span> */}
