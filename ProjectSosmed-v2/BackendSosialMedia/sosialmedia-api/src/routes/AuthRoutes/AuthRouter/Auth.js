...
router.post("/resend-code", postRateLimiter, async (req, res) => {
    ...
    try {
        ...
        // send otp
        try {
            await sgMail.send(ms(cleanEmail, code, "OTP"));
        } catch (e) {
            console.log(e);
            return res.status(500).json({ message: "Failed to send code" });
        }
...
router.post("/reset-password", postRateLimiter, async (req, res) => {
    ...
    // joi validation
    const { error, value } = passwordValidation.validate({ password, confirmPassword }, { stripUnknown : true });
    if (error) {
        const errors = {};
        error.details.forEach(e => {
            const field = e.path[0];
...
router.post("/upload-avatar", JWTVerification, postRateLimiter, upload.single("file"), async (req, res) => { 
    ...
    try {
        ...
        let uploadFile;
        try {
            uploadFile = await googleDriveConfig.files.create({
                requestBody: {
        ...
        if (user.avatar?.fileId && user.avatar.fileId != "1A8JIDhPsQ6s1zRAYjFl_qXq0qUBE5Ah6") {
            try { 
                await googleDriveConfig.files.delete({ fileId : user.avatar.fileId})
            } catch (e) { 
