...
// Google Drive Config
const googleDriveConfig = require("../../../Services/GoogleDriveConfig/googleDriveConfig");
    ...
    try {
        ...
        const savedFile = await Promise.all([
            (async () => { 
                ...
                try {
                    const uploadFile = await googleDriveConfig.files.create({
                        requestBody: {
                    ...
                    await googleDriveConfig.permissions.create(
                        {
        ...
        try { 
            await googleDriveConfig.files.delete({ fileId: uploadFileId });
        } catch (e) {
...
router.delete("/delete-comment/:commentId", JWTVerification, postRateLimiter, async (req, res) => { 
    ...
    try {
        ...
        await Promise.all(replies.map(reply => limit(async () => { 
            if (reply.file?._id) {
                try {
                    await googleDriveConfig.files.delete({ fileId: reply.file.fileId });
                } catch (e) {
        ...
        if (comment.file) {
            try {
                await googleDriveConfig.files.delete({ fileId : comment.file.fileId});
            } catch (e) {
...
router.delete("/:replycommentId/delete-replycomment", JWTVerification, postRateLimiter, async (req, res) => { 
    ...
    try {
        ...
        if (replycomment.file) { 
            try { 
                await googleDriveConfig.files.delete({fileId : replycomment.file.fileId})
            } catch (e) {
