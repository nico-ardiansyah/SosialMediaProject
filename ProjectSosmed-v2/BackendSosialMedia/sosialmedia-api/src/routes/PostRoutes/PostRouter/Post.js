...
// Google Drive Config
const googleDrvieConfig = require("../../../Services/GoogleDriveConfig/googleDriveConfig");
...
router.post("/upload-post", JWTVerification, postRateLimiter, upload.array("files", 5), async (req, res) => { 
    ...
    try {
        ...
        const newFiles = await Promise.all(files.map(async (file) => { 
            ...
            try {
                const uploadFile = await googleDrvieConfig.files.create({
                    requestBody: {
                ...
                await googleDrvieConfig.permissions.create({
        ...
        for (const id of uploadFiles) {
            try {
                await googleDrvieConfig.files.delete({ fileId: id });
            } catch {
    ...
    try {
        ...
        await Promise.all(comments.map((comment) => limit(async() => {
            ...
            await Promise.all(replies.map((reply) => limit(async() => {
                if (reply.file?._id) {
                    try {
                        await googleDrvieConfig.files.delete({ fileId: reply.file.fileId });
                    } catch (err) {
            ...
            if (comment.file?._id) {
                try {
                    await googleDrvieConfig.files.delete({ fileId: comment.file.fileId });
                } catch (err) {
        ...
        if (post.files?.length > 0) {
            await Promise.all(post.files.map((file) => limit(async() => {
                try {
                    await googleDrvieConfig.files.delete({ fileId: file.fileId });
                } catch (err) {
